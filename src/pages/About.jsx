...
const About = () => {
  ...
  return (
    <div className="w-full">
      ...
      <motion.section
        className="bg-gradient-to-r from-[#442063] via-[#342557] to-[#1d3263] text-white w-full py-16"
        initial={{ opacity: 0 }}
      ...
      >
        ...
        <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-center text-gray-600 mb-18">
          leaders and legacy
        </h3>
        ...
        <div className="max-w-6xl mx-auto px-6">
          {teamMembers.map((member, index) => (
            <motion.div
              ...
              transition={{ duration: 0.8, delay: Math.min(index * 0.3, 0.6) }}
              viewport={{ once: true }}
            >
              {/* Left - Polaroid-Style Card */}
              <motion.div
                className="relative bg-white/10 backdrop-blur-lg shadow-lg rounded-xl p-4 flex flex-col items-center w-64 h-80"
                initial={{ opacity: 0, scale: 0.9 }}
                ...
                viewport={{ once: true }}
              >
                {/* Image with Rounded Rectangle Shape */}
                <motion.img
                  src={member.image}
                  alt={member.name}
                  className="w-50 h-72 object-cover rounded-lg border-2 border-white shadow-md overflow-hidden"
